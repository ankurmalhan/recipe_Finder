...
class App extends Component {
  ...
  findRecipe = async() =>{
    ...
    const response = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${this.state.sdata}`);
  ...
  this.setState({
    rcepaarr: myJson.meals[0],
    imgeee: myJson.meals[0].strMealThumb,
    lodingg: 'lodingg_DONE',
    ingredients: ingredients,
    measures: measures
  });
  console.log(this.state.rcepaarr)
}
...
getIngredients = (value, index) => {
  ...
  render() {   
    ...
    <div id="header">
         ...
         <center><input onChange={()=>this.setName(event)} value={this.state.sdata} placeholder="Enter the Name of the Dish"/>
         <span><button onClick={this.findRecipe}>Get Recipes</button></span>
         <br/>
         <br/>
         {this.state.lodingg == null ? (<h2>Enter Dish Name to Search for it's recipie</h2>): ("")}</center>
    </div>

         {this.state.lodingg == "lodingg_FAILED" ? (<h1>No Such Data</h1>): ("")}

         {this.state.lodingg=="lodingg" ? (<h1>lodingg....</h1>): ("")}

         {this.state.lodingg == "lodingg_DONE"? (
    <div id="container">

      <div id="header1">
         <div></div>
         <div><h1 id="main">{this.state.rcepaarr.strMeal}</h1></div>
      ...
      <div id="description">
        ...
        <div id="left">
         <imgeee src={this.state.imgeee}/>
        </div>

        <div id="right">
         <i>Category of the Meal - </i>{this.state.rcepaarr.strCategory}
         <br/>
         <i>Area of the Meal - </i>{this.state.rcepaarr.strArea}
         <br/>
         <br/>
         ...
         <div id="recipe">
         {this.state.rcepaarr.strInstructions}
         </div>
